    ...
    <AppBar component="nav" position="sticky" sx={{ backgroundColor: 'white' }} elevation={0} style={{ width: '100%', height: '80px' }}>
      <Toolbar>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <img onClick={()=>navigate("/")} src={navImg1} alt="mulheres conectadas" style={{ width: '200px', height: '80px', marginRight: '5px', cursor: 'pointer', '&hover':{opacity : 0.6} }} />
        ...
        <List sx={{ display: 'flex', alignItems: 'center', gap: '10px', marginLeft: 'auto' }}>
          {lista.map((item) => (
            <ListItem key={item.text} sx={{ color: '#ab4f9d'}}>
              ...
              <ListItemButton component={Link} onClick={item.to} to={item.to} sx={{ color: buttonColor, '&:hover': { backgroundColor: 'transparent', color: 'purple' } }}>
